...
func TestILike(t *testing.T) {
	for _, tt := range []struct {
		...
		{
			name: "column name with table alias in backticks",
			in:   "SELECT * FROM `tbl` AS `t` WHERE `t`.`name` ILIKE ?",
			out:  "SELECT * FROM `tbl` AS `t` WHERE Unicode::ToLower(`t`.`name`) LIKE Unicode::ToLower(?)",
		},
		{
			name: "arg ILIKE arg",
			in:   "SELECT 1 WHERE ? ILIKE ?",
			out:  "SELECT 1 WHERE Unicode::ToLower(?) LIKE Unicode::ToLower(?)",
		},
		{
			name: "no ilike",
		...
		{
			name: "table_col ilike",
			in:   "AND le.name ILIKE $p3",
			out:  "AND Unicode::ToLower(le.name) LIKE Unicode::ToLower($p3)",
		},
		{
			name: "ILIKE in literal (single quotas)",
			in:   "SELECT 1 WHERE name = ' name ILIKE ?'",
			out:  "SELECT 1 WHERE name = ' name ILIKE ?'",
		},
		{
			name: "ILIKE in literal (double quotas)",
			in:   "SELECT 1 WHERE name = \" name ILIKE ?\"",
			out:  "SELECT 1 WHERE name = \" name ILIKE ?\"",
		},
	} {
